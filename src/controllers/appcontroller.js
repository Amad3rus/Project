...
export default class AppController {
    ...
    hideMenuOnclick(){
        this.el['hideMenu'].onclick = (e) => {
            e.preventDefault();
            if(this.config.sidebar && !this.el['sidebar'].classList.contains(this.config.animate)){
                this.el['sidebar'].classList.add(this.config.animate);
                this.el['sidebar'].classList.add(this.config.fadeinleft);
                this.el['sidebar'].style.width = '100%';
                this.config.sidebar = false;
                this.el['menuSetinha'].checked = true;
            }else{
                this.el['sidebar'].classList.remove(this.config.animate);
                this.el['sidebar'].classList.remove(this.config.fadeinleft);
        ...
        this.contactsService.fetchContacts()
            .then((contacts) => {
                contacts.forEach((value, index) => {
                    ...
                    `
                    li.onclick = e => this.fetchMessages(value);
        ...
        this.messagesService.fetchMessages(value)
            .then((messages) => {
                if(messages.length > 0){
                    ...
                    this.el['containerChat'].style.background = this.backgroundDoodles;
                    this.el['noContactSelected'].style.display = 'none';
                    ...
                    this.el['containerChat'].style.background = 'rgba(43,44,45,1)';
                    this.el['chat'].style.display = 'flex';
                    this.el['chat'].style.flexDirection = 'column';
                    this.el['chat'].style.justifyContent = 'center';
                    this.el['chat'].style.alignItems = 'center';
    ...
    onRizeWindow(){
        window.addEventListener('resize', (e) => {
            if(e.target.innerWidth <= 1024){
                ...
                this.el['sidebar'].classList.remove(this.config.left);
                this.el['sidebar'].classList.add(this.config.animate);
                this.el['sidebar'].classList.add(this.config.fadeinleft);
                this.el['sidebar'].style.width = '100%';
                this.config.sidebar = false;
