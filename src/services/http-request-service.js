export default class HttpRequestService {
    ...
    get(path){
        return new Promise((resolve, reject) => {
            ...
            this.api.onload = () => {
                (this.api.readyState == 4 && this.api.status == 200) ? resolve(this.api.response) : reject({error: true})
            }
    ...
    post(payload, path){
        return new Promise((resolve, reject) => {
            ...
            this.api.onload = () => {
                (this.api.DONE && this.api.status === 200) ? resolve(this.api.response) : reject({error: true})
            }
