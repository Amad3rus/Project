...
export default class AudioService extends CreateEvent{
    ...
    startAudioRecorder(){
        if(this.available()){
            ...
            this.chunks = [];
            this.recording.addEventListener('dataavailable', e => (e.data.size > 0) ? this.chunks.push(e.data) : []);
            this.recording.addEventListener('stop', e => {
                const blob = new Blob(this.chunks, {type:this.mimeType});
                const filename = `audio${Date.now()}.webm`;
                const file = new File([blob], filename, { type: this.mimeType, lastModified: Date.now() });
